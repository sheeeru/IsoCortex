...
# ---------------------------------------------------------------------------
DEFAULT_CHUNK_SIZE:  int = 350   # target words per chunk (~512 tokens for MiniLM)
DEFAULT_OVERLAP:     int = 38    # word overlap between consecutive chunks (~50 tokens)
DEFAULT_TOKEN_LIMIT: int = 512   # hard limit (CON-1, FR-ING-002 in SRS v5.0)

# Overlap ratio bounds — used for validation
MIN_OVERLAP_RATIO: float = 0.10  # 10%
MAX_OVERLAP_RATIO: float = 0.15  # 15%
...
def chunk_document(
    ...
    *,
    ...
    chunk_size : int
        Target words per chunk. Default 350 (approximately 512 tokens for
        all-MiniLM-L6-v2, following FR-ING-002 in SRS v5.0).

    overlap : int
        Word overlap between consecutive chunks. Default 38
        (approximately 50 tokens, satisfying the FR-ING-002 overlap
        constraint of 50 tokens in SRS v5.0).

    token_limit : int
        Hard maximum tokens per chunk. Default 512 (CON-1, FR-ING-002
        in SRS v5.0).
    ...
    try:
        ...
        # Merge short chunks (FR-ING-002: chunks < 10 tokens merged with adjacent)
        if tokenizer is not None and all_chunks:
            original_count = len(all_chunks)
            merged_texts = _merge_short_chunks(
                [c.text for c in all_chunks], tokenizer, min_tokens=10,
            )
            if len(merged_texts) != original_count:
                logger.debug(
                    "[CHUNKER] Short chunks merged  %d -> %d",
                    original_count, len(merged_texts),
                )
                rebuilt: list[Chunk] = []
                for new_idx, piece in enumerate(merged_texts):
                    rebuilt.append(Chunk(
                        text=piece,
                        chunk_index=new_idx,
                        source_file=all_chunks[0].source_file if all_chunks else "",
                        source_label=all_chunks[0].source_label if all_chunks else "",
                        format_category=all_chunks[0].format_category if all_chunks else "",
                    ))
                all_chunks = rebuilt
...
# ---------------------------------------------------------------------------
# Short chunk merging (FR-ING-002)
# ---------------------------------------------------------------------------
...
def _merge_short_chunks(
    ...
    """
    Merge chunks shorter than min_tokens with their neighbours.

    Short chunks at the start are merged with the next chunk.
    Short chunks at the end are merged with the previous chunk.
    Short chunks in the middle are merged with the previous chunk.

    Parameters
    ----------
    chunks     : list[str] — Candidate chunks.
    tokenizer  : object    — HuggingFace tokenizer (None to skip token check).
    min_tokens : int       — Minimum token count per chunk.
    ...
    if tokenizer is None or min_tokens <= 0:
        return chunks
    ...
    for i, chunk in enumerate(chunks):
        tc = count_tokens(chunk, tokenizer) if tokenizer else len(chunk.split())

        if i == 0 and tc < min_tokens and len(chunks) > 1:
            # First chunk too short — buffer it
            buffer = chunk
            buffer_tokens = tc
            continue

        if buffer and tc < min_tokens and i == len(chunks) - 1:
            # Last chunk too short — merge with buffer
            buffer = buffer + " " + chunk
            buffer_tokens += tc
            if buffer.strip():
        ...
        if buffer:
            # Flush buffer (merge with current)
            combined = buffer + " " + chunk
            merged.append(combined.strip())
            buffer = ""
        elif tc < min_tokens and len(chunks) > 1:
            # Middle chunk too short — buffer it
            buffer = chunk
            buffer_tokens = tc
            continue
        else:
