...
# Index format versioning (SRS Section 7)
INDEX_FORMAT_VERSION: int = 1
INDEX_FORMAT_MAGIC: bytes = b"ISOC"
...
def export_pipeline_result(
    ...
    config            : dict | None       — HNSW configuration.
...
def build_incremental_update(
    ...
    for rec in existing_metadata:
        rec.setdefault("deleted", False)  # ensure deleted field exists (FR-IDX-002)
...
def sha256_of_file(path: Path, chunk_size: int = 65_536) -> str:
    ...
    try:
        ...
        logger.debug("[SERIALIZER] SHA-256 hash failed for %s: %s", path.name, exc)
        return ""
