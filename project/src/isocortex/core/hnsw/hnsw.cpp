...
static void test_soft_delete()
{
    ...
    auto vectors = random_vectors(N, DIM, 444);
    ...
    auto results_before = idx.search(vectors.data(), K);
    ASSERT_TRUE(results_before.size() == K);
    ASSERT_EQ(results_before[0].id, 0);

    // Soft delete vector 0
    ASSERT_TRUE(idx.soft_delete(0));
    ASSERT_EQ(idx.deleted_count(), 1u);
    ASSERT_EQ(idx.active_count(), N - 1);

    // Search should skip deleted vector
    auto results_after = idx.search(vectors.data(), K);
    ...
    ASSERT_TRUE(!idx.soft_delete(9999));

    // Restore vector
    ASSERT_TRUE(idx.restore(0));
    ASSERT_EQ(idx.deleted_count(), 0u);

    // Search should find it again
    auto results_restored = idx.search(vectors.data(), K);
    ASSERT_EQ(results_restored[0].id, 0);
}
...
static void test_save_load()
{
    ...
    // Save — use temp directory for cross-platform compatibility
    const char *tmp_dir = std::getenv("TMPDIR");
    if (!tmp_dir) tmp_dir = std::getenv("TEMP");
    if (!tmp_dir) tmp_dir = std::getenv("TMP");
    if (!tmp_dir) tmp_dir = "/tmp";
    std::string tmp_path = std::string(tmp_dir) + "/isocortex_test_index.bin";
    idx1.save(tmp_path.c_str());

    // Load
    HnswIndex idx2(cfg);
    idx2.load(tmp_path.c_str());

    ASSERT_EQ(idx2.size(), N);
    ASSERT_TRUE(idx2.is_built());
    ASSERT_EQ(idx2.deleted_count(), 2u);
    ASSERT_EQ(idx2.active_count(), N - 2);
    ASSERT_EQ(idx2.dim(), DIM);
    ASSERT_EQ(idx2.entry_point(), idx1.entry_point());
    ASSERT_EQ(idx2.max_level(), idx1.max_level());

    // Search should work the same
    auto q = vectors.data();
    auto r1 = idx1.search(q, K);
    auto r2 = idx2.search(q, K);
