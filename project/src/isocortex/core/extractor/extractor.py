...
Supported formats (Section 1.3):
  plain_text   -> .txt  .md  .log  .rst  .cfg  .ini
  source_code  -> .py   .cpp .c   .h   .js  .ts  .java  .go  .rs  .rb
  pdf          -> .pdf
  word         -> .docx .odt
  ...
  email        -> .eml
  web          -> .html .htm
  image        -> .png  .jpg  .jpeg  .tiff  .bmp  .gif  .webp  (OCR via Tesseract)
...
class ExtractionResult:
    ...
    file_hash_sha256 : str                  — SHA-256 of raw file bytes.
    """
    absolute_path:   Path
    format_category: str
    extension:       str
...
def extract(scanned_file: object) -> ExtractionResult:
    ...
    try:
        result.file_hash_sha256 = _hash_of_file(path)
        ...
        if ext_lower == ".odt":
            ...
            extractor_fn = _extract_data
        elif ext_lower == ".json":
            extractor_fn = _extract_data
        elif ext_lower == ".tsv":
            extractor_fn = _extract_data
        elif ext_lower in (".png", ".jpg", ".jpeg", ".tiff", ".bmp", ".gif", ".webp"):
            extractor_fn = _extract_image
        else:
...
def _hash_of_file(path: Path, chunk_size: int = 65_536) -> str:
    ...
    try:
        ...
        logger.debug(
            "[EXTRACTOR] SHA-256 hash failed for %s: %s", path.name, exc,
        )
        return ""
...
def _extract_spreadsheet(path: Path, result: ExtractionResult) -> None:
    """Dispatch spreadsheet extraction by extension (XLSX/XLS/ODS only).

    CSV is now handled by the 'data' category via _extract_data.
    """
    ext = path.suffix.lower()
    if ext == ".xls":
        _extract_xls(path, result)
    elif ext == ".ods":
        _extract_ods(path, result)
    else:
...
# ---------------------------------------------------------------------------
# Format dispatch table (FR-1)
# ---------------------------------------------------------------------------
_DISPATCH: dict[str, Callable] = {
    "plain_text":   _extract_plain_text,
    "source_code": _extract_plain_text,
    "pdf":         _extract_pdf,
    "word":        _extract_docx,       # .docx handled here; .odt via extension override
    "presentation": _extract_pptx,       # .pptx handled here; .odp via extension override
    "spreadsheet": _extract_spreadsheet, # .xlsx handled here; .xls/.ods via extension override
    "email":       _extract_eml,
    "web":         _extract_html,
    "data":        _extract_data,
...
# ---------------------------------------------------------------------------
# Format extractors — Data (CSV/TSV/JSON)
# ---------------------------------------------------------------------------

def _extract_data(path: Path, result: ExtractionResult) -> None:
    """Extract data from .csv/.tsv/.json files."""
    ext = path.suffix.lower()
    if ext == ".tsv":
        _extract_tsv(path, result)
    elif ext == ".json":
        _extract_json(path, result)
    else:
        _extract_csv(path, result)


def _extract_tsv(path: Path, result: ExtractionResult) -> None:
    ...
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            ...
            rows = []
            for row in reader:
                pairs = [f"{h}: {v}" for h, v in row.items() if v and v.strip()]
                if pairs:
                    rows.append(" | ".join(pairs))
            if rows:
                _add_chunk(result, "\n".join(rows), source_label=f"Data: {path.name}")
            else:
                _fail(result, "TSV file contains no extractable data")
    except Exception as exc:
        _fail(result, f"TSV parsing failed: {exc}")


def _extract_json(path: Path, result: ExtractionResult) -> None:
    """
    Extract text from JSON files.

    Strategy: If the JSON is an array of objects, linearize each object
    as "Key: Value" pairs (similar to spreadsheet extraction).
    If it's a single object, treat it as one chunk.
    Otherwise, stringify.
    """
    ...
    try:
        ...
        _fail(result, f"JSON parsing failed: {exc}")
        return
    ...
    if isinstance(data, list):
        ...
        if not rows:
            _fail(result, "JSON array contains no extractable data")
            return
        ...
        if pairs:
            _add_chunk(result, "\n".join(pairs), source_label=f"Data: {path.name}")
        else:
            _fail(result, "JSON object contains no extractable data")
    else:
        text = str(data)
        if text.strip():
            _add_chunk(result, text, source_label=f"Data: {path.name}")
        else:
            _fail(result, "JSON file contains no extractable data")


# ---------------------------------------------------------------------------
# Format extractors — Image (OCR via Tesseract)
# ---------------------------------------------------------------------------

def _extract_image(path: Path, result: ExtractionResult) -> None:
    """
    Extract text from an image via OCR using Tesseract (pytesseract + Pillow).

    Performs lazy imports of ``PIL.Image`` and ``pytesseract`` so that the
    extractor module itself remains importable even when the OCR dependencies
    are not installed.  If the dependencies are missing, extraction fails with
    a clear install-time hint.

    Workflow
    -------
    1. Open the image with Pillow.
    2. Run ``pytesseract.image_to_string`` to obtain full-page text.
    3. Run ``pytesseract.image_to_data`` to collect per-word bounding boxes
       and confidence scores for diagnostics.
    4. Clean the recognised text (collapse excessive whitespace).
    5. Emit a single chunk with ``source_label="Image: {filename}"``.
    6. If no usable text is found, fail with a descriptive message.
    """
    # -- lazy imports (dependencies may not be installed) --------------------
    try:
        ...
        _fail(
            ...
            "(and ensure Tesseract OCR binaries are on your PATH).",
        )
        return
    ...
    try:
        ...
        _fail(result, f"Failed to open image {path.name}: {exc}")
        return

    width, height = img.size
    logger.debug(
        "[EXTRACTOR] Image opened for OCR: %s  dimensions=%dx%d",
        path.name, width, height,
    )

    # -- run OCR -------------------------------------------------------------
    try:
        raw_text: str = pytesseract.image_to_string(img, lang="eng")
        ocr_data = pytesseract.image_to_data(img, lang="eng", output_type=pytesseract.Output.DICT)
    except Exception as exc:
        _fail(result, f"Tesseract OCR failed on {path.name}: {exc}")
        return
    ...
    confidences = [
        ...
        if c not in ("-1", "0", "", None) and str(c).strip().lstrip("-").isdigit()
    ]
    if confidences:
        ...
        logger.debug(
            "[EXTRACTOR] OCR confidence for %s: words=%d  avg=%.1f  min=%d  max=%d",
            path.name, len(confidences), avg_conf, min_conf, max_conf,
        )
    else:
        logger.debug(
            "[EXTRACTOR] OCR produced no confidence data for %s", path.name,
        )

    # -- clean text ----------------------------------------------------------
    import re
    cleaned = re.sub(r"[ \t]{2,}", " ", raw_text)       # collapse inline whitespace
    ...
    if not cleaned:
        _fail(
            result,
            f"OCR extracted no readable text from image {path.name} "
            f"({width}x{height}).  The image may contain no text, handwriting, "
            "or text that Tesseract cannot recognise.",
        )
        return

    _add_chunk(result, cleaned, source_label=f"Image: {path.name}")
